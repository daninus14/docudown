...
(defun thing-names-class-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'class))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'class)))
...
(defun  conditionp (thing)
  ...
  (typep thing 'condition))

(defun thing-names-condition-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'condition))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'condition)))
  (let ((it nil))
...
(defun thing-names-constant-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'constant))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'constant)))
  (and (symbolp doc-holder)
       (boundp doc-holder)
       (constantp doc-holder)))

(defun thing-names-function-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'function))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'function)))
  (and (symbolp doc-holder)
       (fboundp doc-holder)
...
(defun thing-names-generic-function-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'generic-function))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'generic-function)))
  (and (symbolp doc-holder)
       (fboundp doc-holder)
...
(defun thing-names-macro-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'macro))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'macro)))
  (and (symbolp doc-holder)
       (macro-function doc-holder)))

(defun thing-names-setf-function-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'setf-function))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'setf-function)))
...
(defun thing-names-slot-accessor-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'slot-accessor))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'slot-accessor)))
  (and (symbolp doc-holder)
       (fboundp doc-holder)
       (typep (symbol-function doc-holder) 'standard-generic-function)
       (some (lambda (m)
...
(defun thing-names-structure-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'structure))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'structure)))
  (and (symbolp doc-holder)
       (let ((class (find-class doc-holder nil))) 
	 (and class 
...
(defun thing-names-variable-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'variable))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'variable)))
  (and (symbolp doc-holder)
       (boundp doc-holder)
       (not (constantp doc-holder))))

(defun thing-names-type-p (doc-holder)
  (maybe-symbol-is-p doc-holder 'type))

(defmethod maybe-symbol-is-p (doc-holder (what (eql 'type)))
  (and (symbolp doc-holder)
       ...
       (or (excl:normalize-type 
	    doc-holder :loud (lambda (&rest r)
			       (declare (ignore r))
			       (return-from maybe-symbol-is-p nil)))
	   t)))
